...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # Janus-Pro image generation requests use VQ image-token embeddings during
    # decode and pair conditional/unconditional rows for CFG.
    janus_image_generation: Optional[List[bool]] = None
    janus_image_pair_ids: Optional[List[Optional[str]]] = None
    janus_image_pair_roles: Optional[List[Optional[str]]] = None
    janus_image_cfg_weights: Optional[List[float]] = None
    janus_image_token_nums: Optional[List[int]] = None
    janus_image_img_sizes: Optional[List[int]] = None
    ...
    def init_new(
        ...
        janus_image_generation = []
        janus_image_pair_ids = []
        janus_image_pair_roles = []
        janus_image_cfg_weights = []
        janus_image_token_nums = []
        janus_image_img_sizes = []
        ...
        for req in batch.reqs:
            ...
            janus_image_generation.append(enabled)
            janus_image_pair_ids.append(custom_params.get("janus_pair_id"))
            janus_image_pair_roles.append(custom_params.get("janus_pair_role"))
            janus_image_cfg_weights.append(
                float(custom_params.get("janus_cfg_weight", 5.0))
            )
            janus_image_token_nums.append(
                int(custom_params.get("janus_image_token_num_per_image", 576))
            )
            janus_image_img_sizes.append(int(custom_params.get("janus_img_size", 384)))
            janus_image_patch_sizes.append(
                int(custom_params.get("janus_patch_size", 16))
            )
            janus_input_image_token_ids.append(
                custom_params.get("janus_input_image_token_ids")
            )
            placeholder_id = custom_params.get("janus_input_image_placeholder_id")
        ...
        if any(janus_image_generation):
            ...
            ret.janus_image_pair_ids = janus_image_pair_ids
            ret.janus_image_pair_roles = janus_image_pair_roles
            ret.janus_image_cfg_weights = janus_image_cfg_weights
            ret.janus_image_token_nums = janus_image_token_nums
            ret.janus_image_img_sizes = janus_image_img_sizes
