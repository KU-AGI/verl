...
class ValidationGenerationsLogger:
    ...
    def _log_generations_to_wandb(self, samples, step, wandb): ### Customized for wandb only
        """Log samples to wandb as a table"""

        # Create column names for all samples
        columns = ["step"] + sum(
            [[f"prompt_{i + 1}", f"task1_gen_img_{i + 1}", f"task2_feedback_text_{i + 1}", f"task3_regen_img_{i + 1}", f"task1_score_{i + 1}", f"task2_score_{i + 1}", f"task3_score_{i + 1}"] for i in range(len(samples))], []
        )
