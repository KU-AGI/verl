...
def _is_multi_modality_model(tokenizer_name, revision=None):
    try:
        config_file = _resolve_local_or_cached_file(
            tokenizer_name, "config.json", revision
        )
        with open(config_file) as f:
            return json.load(f).get("model_type") == "multi_modality"
    except (FileNotFoundError, OSError, json.JSONDecodeError, ValueError):
        return False


def _load_janus_fast_tokenizer(tokenizer_name, revision=None):
    """Load Janus tokenizer.json directly to match the HF/verl rollout.

    In the transformers version used by this SGLang env, AutoTokenizer falls
    through to a slow LlamaTokenizer path for Janus-style multi_modality
    checkpoints. That path drops metaspace/newline behavior and changes prompts
    such as "a photo" into "aphoto". Loading tokenizer.json directly preserves
    the same token ids as the Janus HF rollout.
    """
    if not _is_multi_modality_model(tokenizer_name, revision):
        return None

    try:
        tokenizer_config_file = _resolve_local_or_cached_file(
            tokenizer_name, "tokenizer_config.json", revision
        )
        tokenizer_file = _resolve_local_or_cached_file(
            tokenizer_name, "tokenizer.json", revision
        )
        with open(tokenizer_config_file) as f:
            tokenizer_config = json.load(f)
    except (FileNotFoundError, OSError, json.JSONDecodeError, ValueError) as e:
        logger.debug("Janus tokenizer direct-load skipped for %s: %s", tokenizer_name, e)
        return None
    ...
    logger.info(
        "Loading Janus multi_modality tokenizer directly from tokenizer.json for %s",
        tokenizer_name,
    )
    ...
    tokenizer._sglang_janus_fast_tokenizer = True
    return tokenizer
...
def _fix_v5_tokenizer_components(tokenizer, model_name_or_path, revision=None):
    ...
    try:
        config_file = _resolve_local_or_cached_file(
            model_name_or_path, "config.json", revision
        )
        with open(config_file) as f:
    ...
    if model_type == "multi_modality":
        # Janus/Janus-Pro checkpoints intentionally rely on the Llama tokenizer
        # components used by the HF/verl rollout. Restoring raw tokenizer.json
        # components removes spaces/newlines from prompts and changes behavior.
        return
...
def get_tokenizer(
    ...
    **kwargs,
    ...
    try:
        tokenizer = _load_janus_fast_tokenizer(tokenizer_name, tokenizer_revision)
