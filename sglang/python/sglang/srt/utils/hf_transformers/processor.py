...
def get_processor(
    ...
    **kwargs,
    ...
    if getattr(config, "model_type", None) == "multi_modality" and hasattr(
        processor, "tokenizer"
    ):
    ...
    if type(tokenizer).__name__ == _TOKENIZERS_BACKEND and not getattr(
        tokenizer, "_sglang_janus_fast_tokenizer", False
    ):
        logger.warning(
