...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _forward_raw(
        ...
        janus_flags = getattr(forward_batch, "janus_image_generation", None)
        has_janus_image_generation = janus_flags is not None and any(janus_flags)
        can_run_graph = bool(
            mode_check()
            and self.graph_runner
            and self.graph_runner.can_run(forward_batch)
            and (
                not has_janus_image_generation
                or self.server_args.enable_janus_image_cuda_graph
            )
        )
    ...
    @staticmethod
    def _janus_image_cfg_pairs(forward_batch: ForwardBatch) -> List[Tuple[int, int]]:
        flags = getattr(forward_batch, "janus_image_generation", None)
        ...
        for i, enabled in enumerate(flags):
            if not enabled:
                continue
            pair_id = forward_batch.janus_image_pair_ids[i]
            role = forward_batch.janus_image_pair_roles[i]
            if pair_id is None or role not in ("cond", "uncond"):
                continue
            pair_map.setdefault(pair_id, {})[role] = i

        pairs = []
        for pair in pair_map.values():
            if "cond" in pair and "uncond" in pair:
                pairs.append((pair["cond"], pair["uncond"]))
        return pairs

    def _apply_janus_image_cfg(
        self,
        ...
        for cond_i, uncond_i in pairs:
            cfg_weight = forward_batch.janus_image_cfg_weights[cond_i]
            guided = logits[uncond_i] + cfg_weight * (
                logits[cond_i] - logits[uncond_i]
            )
            logits[cond_i].copy_(guided)
            logits[uncond_i].copy_(guided)
        self._mask_janus_image_vocab(logits_output, forward_batch)

    def _mask_janus_image_vocab(
        self,
        logits_output: LogitsProcessorOutput,
        forward_batch: ForwardBatch,
    ) -> None:
        ...
        gen_embed = getattr(self.model, "gen_embed", None)
        image_vocab_size = getattr(gen_embed, "num_embeddings", None)
        if image_vocab_size is None:
            return

        logits = logits_output.next_token_logits
        if image_vocab_size >= logits.shape[-1]:
            return

        gen_mask = torch.tensor(flags, dtype=torch.bool, device=logits.device)
        logits[gen_mask, image_vocab_size:] = torch.finfo(logits.dtype).min

    def _validate_janus_image_token_ids(
        self,
        forward_batch: ForwardBatch,
        next_token_ids: torch.Tensor,
    ) -> None:
        ...
        gen_mask = torch.tensor(flags, dtype=torch.bool, device=next_token_ids.device)
        invalid = gen_mask & (
            (next_token_ids < 0) | (next_token_ids >= image_vocab_size)
        )
        if not invalid.any():
            return

        bad_indices = invalid.nonzero(as_tuple=False).flatten().tolist()
        bad_tokens = next_token_ids[invalid].detach().cpu().tolist()
        bad_roles = [forward_batch.janus_image_pair_roles[i] for i in bad_indices]
        raise RuntimeError(
            "Janus image generation sampled token outside VQ vocab "
            f"(size={image_vocab_size}): "
            f"indices={bad_indices}, roles={bad_roles}, tokens={bad_tokens}"
        )

    def _attach_janus_image_outputs(
        self,
        logits_output: LogitsProcessorOutput,
        forward_batch: ForwardBatch,
        next_token_ids: torch.Tensor,
        ...
        if not hasattr(self.model, "decode_janus_image_tokens"):
            return

        token_buffers = getattr(self, "_janus_image_token_buffers", None)
        if token_buffers is None:
            token_buffers = {}
            self._janus_image_token_buffers = token_buffers
        pair_rids = getattr(self, "_janus_image_pair_rids", None)
        if pair_rids is None:
            pair_rids = {}
            self._janus_image_pair_rids = pair_rids

        flags = getattr(forward_batch, "janus_image_generation", None) or []
        for i, enabled in enumerate(flags):
            if enabled:
                rid = forward_batch.rids[i]
                pair_id = forward_batch.janus_image_pair_ids[i]
                if pair_id is not None:
                    pair_rids.setdefault(pair_id, set()).add(rid)
                token_buffers.setdefault(rid, []).append(int(next_token_ids[i].item()))

        image_base64 = [None] * len(next_token_ids)
        pair_uncond_indices = {cond_i: uncond_i for cond_i, uncond_i in pairs}
        for cond_i, enabled in enumerate(flags):
            if not enabled or forward_batch.janus_image_pair_roles[cond_i] != "cond":
                continue

            cond_rid = forward_batch.rids[cond_i]
            output_ids = token_buffers.get(cond_rid, [])
            target_len = forward_batch.janus_image_token_nums[cond_i]
            grid_size = (
                forward_batch.janus_image_img_sizes[cond_i]
                // forward_batch.janus_image_patch_sizes[cond_i]
            )
            if len(output_ids) < target_len:
                continue

            output_ids = output_ids[:target_len]
            if target_len == grid_size * grid_size:
                encoded = self.model.decode_janus_image_tokens(
                    output_ids,
                    img_size=forward_batch.janus_image_img_sizes[cond_i],
                    patch_size=forward_batch.janus_image_patch_sizes[cond_i],
                )
                image_base64[cond_i] = encoded
                uncond_i = pair_uncond_indices.get(cond_i)
                if uncond_i is not None:
                    image_base64[uncond_i] = encoded

            pair_id = forward_batch.janus_image_pair_ids[cond_i]
            for rid in pair_rids.pop(pair_id, {cond_rid}):
                token_buffers.pop(rid, None)

        if any(flags):
            if logits_output.customized_info is None:
                logits_output.customized_info = {}
            logits_output.customized_info["janus_image_base64"] = image_base64
    ...
    def sample(
        ...
        """
        janus_image_pairs = self._janus_image_cfg_pairs(forward_batch)
        self._apply_janus_image_cfg(logits_output, forward_batch, janus_image_pairs)
        ...
        for cond_i, uncond_i in janus_image_pairs:
            next_token_ids[uncond_i] = next_token_ids[cond_i]
        self._validate_janus_image_token_ids(forward_batch, next_token_ids)
        self._attach_janus_image_outputs(
            logits_output, forward_batch, next_token_ids, janus_image_pairs
        )
