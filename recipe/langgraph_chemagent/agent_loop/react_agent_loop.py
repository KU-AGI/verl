...
from recipe.langgraph_chemagent.agent_loop.agent_loop import AgentLoopBase, AgentLoopOutput
from recipe.langgraph_chemagent.chat_model import (
    ChatModel,
...
class ReactAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        model = ChatModel(
            ...
            enable_think=getattr(rollout, "enable_think", False),
        )

        tool_names = [t.name for t in self.tools]
        logger.info(f"Binding {len(self.tools)} tools: {tool_names}")
        ...
        output = convert_to_agent_output(state["messages"], rollout.response_length, tool_schemas=self._get_tool_schemas())
        return output

    def _get_tool_schemas(self) -> dict | None:
        """Return tool JSON schemas for reward computation. Override in subclasses."""
        return None
