...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.STEPWISE_GRPO:
        # Initialize the mask for GRPO calculation
        grpo_calculation_mask = data.batch["response_mask"]

        # Call compute_stepwise_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_stepwise_grpo_outcome_advantage(
            data=data,
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.STEPCUMUL_GRPO:
        # Initialize the mask for GRPO calculation
        grpo_calculation_mask = data.batch["response_mask"]

        # Call compute_stepcumul_grpo_outcome_advantage with parameters matching its definition
        advantages, returns, new_response_mask = core_algos.compute_stepcumul_grpo_outcome_advantage(
            data=data,
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["advantages"] = advantages
        data.batch["response_mask"] = new_response_mask
        data.batch["returns"] = returns
    else:
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Store generated outputs
            output_ids = test_output_gen_batch.batch["responses"]
            output_texts = [self.tokenizer.decode(ids, skip_special_tokens=False).replace("<|endoftext|>", "") for ids in output_ids]
            sample_outputs.extend(output_texts)
