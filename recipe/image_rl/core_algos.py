...
@register_adv_est(AdvantageEstimator.GRPO_TASK_SKIP)  # or simply: @register_adv_est("grpo_task_skip")
def compute_grpo_task_skip_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    scores = token_level_rewards.sum(dim=-1)
    scores_length = token_level_rewards.size(1)
