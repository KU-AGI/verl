...
async def janus_generate_image_request(request: Request):
    """Generate Janus-Pro image tokens/images with CFG on the text-generation runtime."""
    ...
    if processor is None or tokenizer is None:
        return _create_error_response(
            ValueError("Janus image generation requires tokenizer and processor.")
        )

    mode = body.get("mode", "text2image")
    source_image_token_ids = body.get(
        "input_image_token_ids", body.get("source_image_token_ids")
    )
    edit_mode = (
        mode in ("edit", "image_edit", "regen")
        or source_image_token_ids is not None
    )

    prompt = body.get("prompt", body.get("text", body.get("input_prompt")))
    conversation = body.get("conversation")
    if conversation is None:
        if edit_mode:
            feedback = body.get(
                "feedback", body.get("edit_instruction", body.get("instruction", ""))
            )
            ...
            if not prompt and not feedback:
                return _create_error_response(
                    ValueError(
                        "Janus edit mode requires `prompt`/`input_prompt` or `feedback`."
                    )
                )
            edit_content = (
                f"{processor.image_start_tag}{processor.image_tag}"
                f"{processor.image_end_tag}\n"
                "Please edit the image as instructed.\n"
                "The FEEDBACK will be given as multiple steps (Step 1, Step 2, ...). "
                "You MUST apply all steps in order and produce a final image reflecting "
                "all changes.\n"
                f"INPUT_PROMPT: {prompt}\n"
                f"FEEDBACK: \n{feedback or 'No need to generate feedback.'}"
            )
            conversation = [
                {"role": "<|User|>", "content": edit_content},
                {"role": "<|Assistant|>", "content": ""},
            ]
            if "system_prompt" not in body:
                body["system_prompt"] = (
                    "You are a strict image editing assistant.\n"
                    "Your task is to revise a failed generated image according to "
                    "the user's instruction and the original generation intent.\n\n"
                    "INPUT FORMAT:\n"
                    "1. The source image is located between {image_start_tag} and "
                    "{image_end_tag}.\n"
                    "2. The original text-to-image generation prompt will be provided "
                    "after the keyword 'INPUT_PROMPT:'\n"
                    "3. Step-by-step feedback will be provided after the keyword "
                    "'FEEDBACK:'.\n"
                    "   - The feedback will be a sequence of instructions, each "
                    "starting with 'Step X:' (e.g., 'Step 1:', 'Step 2:', ...).\n"
                    "   - You MUST follow ALL steps in order and produce a final "
                    "image that satisfies the entire sequence, not just an "
                    "intermediate step.\n\n"
                    "CRITICAL RULES:\n"
                    "1. You MUST Look at the image between {image_start_tag} and "
                    "{image_end_tag} as the ground truth.\n"
                    "2. Preserve the background, objects, and style from the input "
                    "image unless explicitly asked to change them.\n"
                    "3. Do NOT generate a completely new image from scratch.\n"
                    "4. **You MUST strictly maintain the spatial layout and "
                    "composition of the source image.**\n"
                    "5. You MUST also reference the INPUT_PROMPT as the original "
                    "intended content of the image, but the visible source image "
                    "remains the primary ground truth.\n"
                    "6. When applying FEEDBACK, carefully execute each step one by "
                    "one while keeping previous changes consistent, and ensure the "
                    "final result reflects all steps combined."
                ).format(
                    image_start_tag=processor.image_start_tag,
                    image_end_tag=processor.image_end_tag,
                )
        else:
            if not isinstance(prompt, str) or not prompt:
                return _create_error_response(
                    ValueError("`prompt` or `text` must be a non-empty string.")
                )
            conversation = [
    ...
    if hasattr(processor, "apply_sft_template_for_multi_turn_prompts"):
        ...
        seps = ["\n\n", "<｜end▁of▁sentence｜>"]
        sft_format = (system_prompt + seps[0]) if system_prompt else ""
    ...
    janus_tokenizer = getattr(tokenizer_manager, "_janus_generation_tokenizer", None)
    if janus_tokenizer is None:
        from sglang.srt.utils.hf_transformers import get_tokenizer

        server_args = tokenizer_manager.server_args
        janus_tokenizer = get_tokenizer(
        ...
        tokenizer_manager._janus_generation_tokenizer = janus_tokenizer
    num_images = int(body.get("n", body.get("parallel_size", 1)))
    cfg_weight = float(body.get("cfg_weight", 5.0))
    ...
    if num_images < 1:
        return _create_error_response(ValueError("`n` must be at least 1."))

    def _normalize_image_token_batches(value):
        if value is None:
            return None
        if not isinstance(value, list) or len(value) == 0:
            raise ValueError(
                "`input_image_token_ids` must be a non-empty list of token ids."
            )
        batches = value if isinstance(value[0], list) else [value]
        if len(batches) not in (1, num_images):
            raise ValueError(
                "`input_image_token_ids` must contain either one 576-token image "
                "or one 576-token image per requested output."
            )
        normalized = []
        for tokens in batches:
            if len(tokens) != image_token_num:
                raise ValueError(
                    "`input_image_token_ids` must contain exactly "
                    f"{image_token_num} tokens per image."
                )
            normalized.append([int(token_id) for token_id in tokens])
        return normalized

    try:
        source_image_token_batches = _normalize_image_token_batches(
            source_image_token_ids
        )
    except ValueError as e:
        return _create_error_response(e)
    ...
    if source_image_token_batches is not None:
        if not tokenizer_manager.server_args.disable_radix_cache:
            return _create_error_response(
                ValueError(
                    "Janus edit mode currently requires --disable-radix-cache. "
                    "The input image is injected as embeddings over repeated "
                    "placeholder ids, so prefix cache reuse would condition on "
        ...
        if placeholder_count != 1:
            return _create_error_response(
                ValueError(
                    "Janus edit mode requires exactly one input image placeholder "
                    "in the prompt."
                )
            )
        prompt_ids = expanded_prompt_ids

    def _make_edit_uncond_ids(input_ids):
        uncond_ids = list(input_ids)
        start_idx = -1
        for idx in range(len(uncond_ids) - 1):
            if uncond_ids[idx] == 100593 and uncond_ids[idx + 1] == 185:
                start_idx = idx
                break
        try:
            end_idx = uncond_ids.index(100602)
        except ValueError:
            end_idx = -1

        if start_idx >= 0 and end_idx >= 0 and start_idx < end_idx:
            content_start = start_idx + 1
            content_end_excl = min(end_idx + 2, len(uncond_ids) - 1)
            uncond_ids[content_start:content_end_excl] = [processor.pad_id] * (
                content_end_excl - content_start
            )
            return uncond_ids

        keep_ids = {int(processor.pad_id), image_placeholder_id}
        for idx in range(1, len(uncond_ids) - 1):
            if uncond_ids[idx] not in keep_ids and uncond_ids[idx] < 100000:
                uncond_ids[idx] = processor.pad_id
        return uncond_ids
    ...
    for image_idx in range(num_images):
        ...
        image_token_batch = (
            None
            if source_image_token_batches is None
            else source_image_token_batches[
                image_idx if len(source_image_token_batches) > 1 else 0
            ]
        )
        for role in ("cond", "uncond"):
            input_ids = list(prompt_ids)
            if role == "uncond" and len(input_ids) > 2:
                if image_token_batch is None:
                    input_ids[1:-1] = [processor.pad_id] * (len(input_ids) - 2)
                else:
                    input_ids = _make_edit_uncond_ids(input_ids)
            batch_input_ids.append(input_ids)
            batch_rids.append(uuid.uuid4().hex)
            custom_params = {
                "janus_image_generation": True,
                "janus_pair_id": pair_id,
                "janus_pair_role": role,
                "janus_cfg_weight": cfg_weight,
                "janus_image_token_num_per_image": image_token_num,
                "janus_img_size": img_size,
                "janus_patch_size": patch_size,
            }
    ...
    try:
        outputs = await tokenizer_manager.generate_request(obj, request).__anext__()
    except ValueError as e:
        logger.error(f"[http_server] Error: {e}")
        return _create_error_response(e)
    ...
    images = []
    for image_idx in range(num_images):
        output = outputs[image_idx * 2]
        meta_info = output.get("meta_info", {})
        image_base64 = meta_info.get("janus_image_base64")
        if isinstance(image_base64, list):
            image_base64 = next(
                (item for item in reversed(image_base64) if item is not None), None
            )
        image_token_ids = output.get("output_ids")
        image_token_logprobs = None
        if return_logprob:
            image_token_logprobs = []
...
@app.post("/janus/update_weights_from_flat_file")
@auth_level(AuthLevel.ADMIN_OPTIONAL)
async def janus_update_weights_from_flat_file(request: Request):
    """Update Janus weights from a local flat tensor file.

    The outer verl process exports a flat tensor and metadata into /dev/shm.
    Loading and serializing named tensors inside the SGLang server process keeps
    multiprocessing auth keys consistent with the scheduler process.
    """
    ...
    bucket_bytes = int(body.get("bucket_bytes", 512 << 20))
    torch_empty_cache = bool(body.get("torch_empty_cache", False))

    flat_tensor = torch.load(file_path, map_location="cpu", mmap=True, weights_only=True)
    metadata = torch.load(metadata_path, map_location="cpu", weights_only=False)
    tensors_meta = metadata.get("tensors", metadata)
    if not isinstance(tensors_meta, list):
        return ORJSONResponse(
            {
                "success": False,
                "message": f"Invalid rollout weight metadata type: {type(tensors_meta)}",
            },
            status_code=HTTPStatus.BAD_REQUEST,
        )
    ...
    async def flush_bucket(flush_cache: bool):
        nonlocal bucket, bucket_nbytes
        if not bucket:
            return True, "No tensors to update."
        obj = UpdateWeightsFromTensorReqInput(
            serialized_named_tensors=[
                MultiprocessingSerializer.serialize(bucket, output_str=True)
            ],
    ...
    for item in tensors_meta:
        ...
        numel = int(item.get("numel", int(np.prod(shape))))
        tensor = flat_tensor[offset : offset + numel].view(shape).clone()
        offset += numel
        bucket.append((name, tensor))
        bucket_nbytes += tensor.numel() * tensor.element_size()
        total_tensors += 1
        if bucket_nbytes >= bucket_bytes:
            success, message = await flush_bucket(flush_cache=False)
            if not success:
                return ORJSONResponse(
    ...
    success, message = await flush_bucket(flush_cache=True)
    if not success:
        return ORJSONResponse(
            {"success": False, "message": message},
            status_code=HTTPStatus.BAD_REQUEST,
