...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        dataset = dataset.map(map_messages, fn_kwargs={"split": split}, load_from_cache_file=False, num_proc=20)
        ...
        dataset = dataset.map(map_answer, load_from_cache_file=False, num_proc=20)
        concatenated.append(dataset)

    concatenated = concatenate_datasets(concatenated)

    if args.train:
        dataset = concatenated.map(function=append_info, load_from_cache_file=False, num_proc=20)
