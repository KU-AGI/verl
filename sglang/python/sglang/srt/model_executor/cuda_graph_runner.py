...
class CudaGraphRunner:
    ...
    def __init__(
        ...
        *,
        ...
        requested_janus_image_cuda_graph = getattr(
            model_runner.server_args, "enable_janus_image_cuda_graph", False
        )
        self.enable_janus_image_cuda_graph = bool(
            requested_janus_image_cuda_graph
            and hasattr(model_runner.model, "prepare_gen_img_embeds")
            and hasattr(model_runner.model, "gen_head")
        )
        if requested_janus_image_cuda_graph and not self.enable_janus_image_cuda_graph:
            logger.warning(
                "--enable-janus-image-cuda-graph was set, but this model does not "
                "expose Janus image-generation modules; skipping the extra graph."
            )
        if self.enable_janus_image_cuda_graph:
            log_info_on_rank0(
                logger,
                "Capture Janus image-generation cuda graph variant. "
                "Image-generation batches must not be mixed with text batches.",
            )
    ...
    @staticmethod
    def _is_janus_image_generation_batch(forward_batch: ForwardBatch) -> bool:
        flags = getattr(forward_batch, "janus_image_generation", None)
        return flags is not None and any(flags)

    @staticmethod
    def _is_janus_image_generation_only_batch(forward_batch: ForwardBatch) -> bool:
        flags = getattr(forward_batch, "janus_image_generation", None)
        return flags is not None and bool(flags) and all(flags)

    @staticmethod
    def _compose_graph_variant(
        graph_variant: Optional[str], lora_variant: Optional[str]
    ) -> Optional[str]:
        labels = [label for label in (graph_variant, lora_variant) if label]
        return "_".join(labels) if labels else None

    def _resolve_graph_variant(self, forward_batch: ForwardBatch) -> Optional[str]:
        graph_variant = (
            "janus_image"
            if self._is_janus_image_generation_batch(forward_batch)
            else None
        )
        return self._compose_graph_variant(
            graph_variant, self._resolve_lora_variant(forward_batch)
        )
    ...
    def can_run(self, forward_batch: ForwardBatch):
        ...
        if forward_batch.replace_embeds is not None:
            return False
        if self._is_janus_image_generation_batch(forward_batch):
            if not self.enable_janus_image_cuda_graph:
                return False
            if not self._is_janus_image_generation_only_batch(forward_batch):
                return False
    ...
    def capture(self) -> None:
        ...
        def _capture_one_stream(stream_idx: Optional[int] = None):
            ...
            graph_variants = [(None, False)]
            if self.enable_janus_image_cuda_graph:
                graph_variants.append(("janus_image", True))
            for i, bs in enumerate(capture_range):
                ...
                for graph_variant, janus_image_generation in graph_variants:
                    for variant_label, variant_has_lora in lora_variants:
                        ...
                        with patch_model(
                            ...
                            (
                                ...
                                forward,
                                stream_idx,
                                janus_image_generation=janus_image_generation,
                            )
                            key = self._make_graph_key(
                                bs,
                                stream_idx,
                                self._compose_graph_variant(
    ...
    def capture_one_batch_size(
        ...
        janus_image_generation: bool = False,
    ):
        ...
        if janus_image_generation:
            forward_batch.janus_image_generation = [True] * bs
