...
from recipe.langgraph_chemagent.agent_loop.agent_loop import AgentLoopOutput, AsyncLLMServerManager
from recipe.langgraph_chemagent.agent_loop.tool_parser import ToolParser
from recipe.langgraph_chemagent.agent_loop.utils import add_generation_prompt_for_gpt_oss, format_gpt_oss_tool_response_manually
...
class ChatModel(BaseChatModel):
    ...
    """Whether to enable thinking mode (e.g. Qwen3 thinking)"""
...
def convert_to_agent_output(
    ...
    # Collect per-turn tool calls from AI messages for reward computation
    turn_tool_calls: list[list] = []
    turn_invalid_tool_calls: list[list] = []
    for msg in messages:
        if msg.type == "ai":
