...
class MultiModalityCausalLM(MultiModalityPreTrainedModel):
    ...
    def _has_janus_image_generation(self, forward_batch: ForwardBatch) -> bool:
        flags = getattr(forward_batch, "janus_image_generation", None)
        return flags is not None and any(flags)

    def _get_janus_generation_input_embeds(
        self,
        ...
        if forward_batch.forward_mode.is_decode() and flags is not None and all(flags):
            input_embeds = self.prepare_gen_img_embeds(input_ids).to(
                embed_tokens.weight.dtype
            )
        else:
            vocab_size = embed_tokens.num_embeddings
            input_embeds = embed_tokens(input_ids.clamp(min=0, max=vocab_size - 1))
            self._apply_janus_input_image_embeds(
                input_ids, input_embeds, forward_batch
            )

            if forward_batch.forward_mode.is_decode() and flags is not None:
                gen_mask = torch.tensor(
                    flags,
                    dtype=torch.bool,
                    device=input_ids.device,
                )
                gen_embeds = self.prepare_gen_img_embeds(input_ids[gen_mask])
                input_embeds[gen_mask] = gen_embeds.to(input_embeds.dtype)
    ...
    def _apply_janus_input_image_embeds(
        self,
        input_ids: torch.LongTensor,
        input_embeds: torch.Tensor,
        forward_batch: ForwardBatch,
    ) -> None:
        image_token_lists = getattr(
            forward_batch, "janus_input_image_token_ids", None
        )
        ...
        if (
            ...
            or not any(tokens is not None for tokens in image_token_lists)
        ):
            return
        ...
        offset = 0
        for batch_idx, seq_len in enumerate(forward_batch.extend_seq_lens_cpu):
            image_token_ids = image_token_lists[batch_idx]
            placeholder_id = placeholder_ids[batch_idx]
            if image_token_ids is None:
                offset += seq_len
                continue
            if placeholder_id is None:
                raise RuntimeError(
                    "Janus input-image conditioning requires a placeholder token id."
                )
            ...
            if placeholder_positions.numel() != len(image_token_ids):
                raise RuntimeError(
                    "Janus input-image conditioning expected "
                    f"{len(image_token_ids)} placeholder tokens, found "
                    f"{placeholder_positions.numel()}."
                )

            image_ids = torch.tensor(
                image_token_ids, dtype=torch.long, device=input_ids.device
            )
            image_embeds = self.prepare_gen_img_embeds(image_ids).to(
                dtype=input_embeds.dtype
            )
            input_embeds[offset + placeholder_positions] = image_embeds
            offset += seq_len
    ...
    def _get_last_hidden_states(
        ...
        for seq_len in forward_batch.extend_seq_lens_cpu:
            last_indices.append(offset + seq_len - 1)
            offset += seq_len
        last_indices = torch.tensor(
            last_indices, dtype=torch.long, device=hidden_states.device
        )
        return hidden_states[last_indices]

    def _get_janus_generation_logits(
        self,
        hidden_states: torch.Tensor,
        input_ids: torch.LongTensor,
        ...
        hidden_states = self._get_last_hidden_states(hidden_states, forward_batch)
        image_logits = self.gen_head(hidden_states)

        vocab_size = self.config.language_config.vocab_size
        next_token_logits = torch.full(
            (image_logits.shape[0], vocab_size),
            torch.finfo(image_logits.dtype).min,
            dtype=image_logits.dtype,
            device=image_logits.device,
        )
        next_token_logits[:, : image_logits.shape[-1]] = image_logits
    ...
    def forward(
        ...
        if self._has_janus_image_generation(forward_batch):
            input_embeds = self._get_janus_generation_input_embeds(
            ...
            if not self.language_model.pp_group.is_last_rank:
                return hidden_states

            gen_logits = self._get_janus_generation_logits(
                hidden_states, input_ids, positions, forward_batch
            )
            if all(forward_batch.janus_image_generation):
                return LogitsProcessorOutput(next_token_logits=gen_logits)

            gen_mask = torch.tensor(
                forward_batch.janus_image_generation,
                dtype=torch.bool,
                device=gen_logits.device,
            )

            logits_output = self.language_model.logits_processor(
                input_ids,
                hidden_states,
                self.language_model.lm_head,
                forward_batch,
            )
            logits_output.next_token_logits[gen_mask] = gen_logits[gen_mask].to(
                logits_output.next_token_logits.dtype
            )
    ...
    def decode_janus_image_tokens(
        ...
        if not isinstance(image_ids, torch.Tensor):
            image_ids = torch.tensor(
                image_ids,
                dtype=torch.int,
                device=self.gen_embed.weight.device,
            )
        image_ids = image_ids.to(device=self.gen_embed.weight.device, dtype=torch.int)
        if image_ids.dim() == 1:
            image_ids = image_ids.unsqueeze(0)

        decoded = self.gen_vision_model.decode_code(
            image_ids,
            shape=[
                image_ids.shape[0],
                8,
        ...
        buffer = io.BytesIO()
        Image.fromarray(decoded[0]).save(buffer, format="PNG")
