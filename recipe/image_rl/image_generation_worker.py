...
from verl.workers.fsdp_workers import ActorRolloutRefWorker, RewardModelWorker, create_device_mesh, get_sharding_strategy, init_device_mesh, get_vl_model_vision_tower
...
class ImageGenerationActorRolloutRefWorker(ActorRolloutRefWorker):
    """
    This worker is used for image generation.
    """
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelImageGenerationActor(
                config=actor_cfg, processor=self.processor, tokenizer=self.tokenizer, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer
            )
