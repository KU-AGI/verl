...
class MessageQueueClient:
    ...
    async def get_sample_count(self) -> int:
        """Get total sample count in queue (async)"""
        future = self.queue_actor.get_sample_count.remote()
        return await asyncio.wrap_future(future.future())

    def get_sample_count_sync(self) -> int:
        """Get total sample count in queue (sync)"""
        return ray.get(self.queue_actor.get_sample_count.remote())
