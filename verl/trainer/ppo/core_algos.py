...
@register_adv_est(AdvantageEstimator.STEPWISE_GRPO)  # or simply: @register_adv_est("stepwise_grpo")
def compute_stepwise_grpo_outcome_advantage(
    data: Any,
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    ...
    try:
        ...
        scores = token_level_rewards.sum(dim=-1)
        advantage = torch.zeros_like(token_level_rewards)
        ...
        with torch.no_grad():
            ...
            for i in range(bsz):
                ...
                step_keys = list(step_rewards.keys()) # Assume the keys are in order. (e.g. step1, step2, ... answer)
                step_values = list(step_rewards.values())
                for step_i, step in enumerate(step_keys):
                    rew_sum = sum(step_values[step_i:])
                    id2score[f"{index[i]}_{step}"].append(torch.tensor(rew_sum))
            for idx in id2score:
                if len(id2score[idx]) == 1:
                    id2mean[idx] = torch.tensor(0.0)
                    id2std[idx] = torch.tensor(1.0)
...
@register_policy_loss("steplevel")  # type: ignore[arg-type]
def compute_policy_loss_steplevel(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
