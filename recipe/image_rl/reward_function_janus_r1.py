...
def get_messages_image_match(prompt: str, img):
    """Janus-R1 style image/prompt alignment judge.

    The image is inserted directly as a multimodal content part instead of
    going through the local <image> placeholder expansion helper.
    """
    messages = [
        {
            "role": "user",
            "content": [
                {
                    "type": "image_url",
                    "image_url": {"url": convert_gen_img_to_base64(img)},
                },
                {
                    "type": "text",
                    "text": (
...
def _normalize_yes_no_token(token: Optional[str]) -> str:
    if token is None:
        return ""
...
def _extract_yes_no_score_from_logprobs(tokens) -> dict[str, Any]:
    yes_prob = 0.0
    no_prob = 0.0
    ...
    pos0 = tokens[0]
    candidates = [pos0] + list(_get_attr_or_key(pos0, "top_logprobs", []) or [])
    seen = set()

    for cand in candidates:
        token = _get_attr_or_key(cand, "token", None)
        logprob = _get_attr_or_key(cand, "logprob", None)
        if token is None or logprob is None:
            continue
        dedupe_key = (str(token), float(logprob))
        ...
        norm = _normalize_yes_no_token(token)
        if norm == "yes":
            yes_prob += prob
        elif norm == "no":
            no_prob += prob

        if len(top_tokens) < 20:
            top_tokens.append(
                {
                    "token": str(token),
                    "norm": norm,
    ...
    if denom <= 0:
        score = 0.5
        status = "no_yes_no_candidate"
    else:
        score = round(float(yes_prob / denom), 4)
        status = "ok"
...
async def get_yes_no_score_with_client(client, messages, model):
    """Return Janus-R1 style P(yes)/(P(yes)+P(no)) from first-token logprobs."""
    extra_body = {
        "top_k": -1,
        "min_p": 0.0,
        "best_of": 1,
    ...
    result = _extract_yes_no_score_from_logprobs(tokens)
    message = getattr(response.choices[0], "message", None)
    result["response_text"] = getattr(message, "content", None)
    return result
...
async def get_yes_no_score(message_builder_fn, *args):
    """Generic yes/no logprob reward fetcher with automatic server fallback."""
    try:
        messages, model = message_builder_fn(*args)
    except Exception as e:
        print(f"[REWARD] Message builder {message_builder_fn.__name__} failed: {type(e).__name__}: {e}")
        return {
            ...
            "no_prob": 0.0,
            "top_tokens": [],
            "status": f"message_builder_error:{type(e).__name__}",
            "response_text": None,
        }
    ...
    for attempt in range(max_attempts):
        ...
        try:
            result = await get_yes_no_score_with_client(client, messages, model)
            manager.record_request_result(sid, success=True)
            return result
        except Exception as e:
            manager.record_request_result(sid, success=False, error=e)
            print(f"[REWARD] yes/no API call failed (attempt {attempt+1}/{max_attempts}): {type(e).__name__}: {e}")
        finally:
            await release_rm_client(sid, is_vlm)

    print(f"[REWARD] All {max_attempts} yes/no attempts failed for {message_builder_fn.__name__}, returning 0.0")
    return {
        "score": 0.0,
        "yes_prob": 0.0,
        "no_prob": 0.0,
...
async def compute_score_single_async(
    ...
    if task_id == 1:
        yes_no = await get_yes_no_score(get_messages_image_match, prompt, gen_img)
        task1_image_score = float(yes_no.get("score", 0.0))
        reward_score = task1_image_score

        reward_extra_info[f"task{task_id}_vqa_reward"] = task1_image_score
        reward_extra_info[f"task{task_id}_vqa_reward_response"] = yes_no.get("response_text")
        reward_extra_info[f"task{task_id}_yes_prob"] = float(yes_no.get("yes_prob", 0.0))
        reward_extra_info[f"task{task_id}_no_prob"] = float(yes_no.get("no_prob", 0.0))
        reward_extra_info[f"task{task_id}_yes_no_status"] = yes_no.get("status")
        reward_extra_info[f"task{task_id}_yes_no_top_tokens"] = yes_no.get("top_tokens", [])

        reward_extra_info[f"task{task_id}_detector_reward"] = 0.0
        reward_extra_info[f"task{task_id}_detector_details"] = []
        reward_extra_info[f"task{task_id}_detector_active"] = 0
        reward_extra_info[f"task{task_id}_detector_count"] = 0
        reward_extra_info[f"task{task_id}_detector_active_score"] = 0
        reward_extra_info[f"task{task_id}_detector_active_only_reward"] = None
        ...
        yes_no = await get_yes_no_score(get_messages_image_match, prompt, regen_img)
        task3_image_score = float(yes_no.get("score", 0.0))
        reward_score = task3_image_score
        task3_edit_if_reward = 0.0

        reward_extra_info[f"task{task_id}_vqa_reward"] = task3_image_score
        reward_extra_info[f"task{task_id}_vqa_reward_response"] = yes_no.get("response_text")
        reward_extra_info[f"task{task_id}_yes_prob"] = float(yes_no.get("yes_prob", 0.0))
        reward_extra_info[f"task{task_id}_no_prob"] = float(yes_no.get("no_prob", 0.0))
        reward_extra_info[f"task{task_id}_yes_no_status"] = yes_no.get("status")
        ...
        reward_extra_info[f"task{task_id}_edit_reward_response"] = "Skipped: Janus-R1 yes/no image-match reward"
        reward_extra_info[f"task{task_id}_detector_reward"] = 0.0
        reward_extra_info[f"task{task_id}_detector_details"] = []
        reward_extra_info[f"task{task_id}_detector_active"] = 0
        reward_extra_info[f"task{task_id}_detector_count"] = 0
